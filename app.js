...
*/
game.party.push(pokemon[4], pokemon[7], pokemon[60])
...
*/
//! ATTEMPTED - NOT RIGHT ANSWER GIVEN

game.party.forEach((party) => {
    party.hp.sort((num1, num2) => {
        return num2 - num1
    })
})
...
//! COMMENT OUT AS VALUE OF GYM.DIFFICULTY CHANGED
...
*/
//! COMMENTED OUT TO SHORTEN CONSOLE
...
pokemon.forEach((starters) => {
    if (starters.starter === true) {
        console.log(starters.name)
    }
})
...
game.gymStatus = () => {
    game.gyms.forEach((gym) => {
        if (gym.completed === true) {
            (game.gymTally.completed = gymTally.completed + 1)
        }
    })
